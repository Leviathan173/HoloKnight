...
public class GUIEditor : Editor {
    ...
    void AddPath(int addIndex = -1, int from = -1, int to = -1) {
        ...
        newPath.cost = (int)(script.graphData.GetNode(from).Position - script.graphData.GetNode(to).Position).sqrMagnitude;
        if (addIndex == -1)
