...
public class AcherManager : MonoBehaviour,IGameManager
{
    public ManagerStatus status { get; private set; }

    [SerializeField] private Acher acher;
    ...
    public void Startup() {
        ...
        _body = acher.GetComponent<Rigidbody2D>();
        _boxCollider = acher.GetComponent<BoxCollider2D>();
        _animator = acher.GetComponent<Animator>();
        _width = acher.GetComponent<SpriteRenderer>().bounds.size.x / 3;
    ...
    public void AddFrontForce(float force = 0) {
        if (force == 0) {
            _body.velocity = new Vector2((Forward.transform.position.x - acher.transform.position.x) * 9, _body.velocity.y);
        } else {
            _body.velocity = new Vector2((Forward.transform.position.x - acher.transform.position.x) * force, _body.velocity.y);
        }
    ...
    public void Turn(float deltaX) {
        ...
        if (!IsAttacking()
            && !_isOnLadder) {
            acher.transform.localScale = new Vector3(Mathf.Sign(deltaX) * acher.transform.localScale.x, acher.transform.localScale.y, acher.transform.localScale.z);
            if (_isFacingRight && Mathf.Sign(deltaX) < 0) {
                //do turn left
                _isFacingRight = !_isFacingRight;
                acher.transform.position = new Vector3(acher.transform.position.x - _width, acher.transform.position.y, acher.transform.position.z);
